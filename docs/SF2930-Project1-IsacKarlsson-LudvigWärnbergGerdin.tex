...
\label{sec:orgdbd5b03}
\subsection{Introduction}
\label{sec:orgfef0a6d}
...
\label{sec:org30eeff3}
\subsection{Residual analysis}
\label{sec:orgf23f425}
...
The normality of residuals therefore ensures that the confidence intervals presented in section \ref{sec:org7cba459}
are valid.

\subsubsection{Fitted Against Residuals}
\label{sec:orge9adb06}
...
\label{sec:org8e7256d}
\subsubsection{Treatment of outliers}
\label{sec:org3c49b40}
...
\label{sec:org8587219}
\subsection{Diagnostics and handling of Multicolinearity}
\label{sec:org77fa2bb}
\newpage
\section{Results}
\label{sec:org7cba459}
\subsection{Significance tests}
\label{sec:orgf108f70}
...
\subsection{Residual analysis}
\label{sec:org66853b6}
\subsubsection{Normality of residuals}
\label{sec:orgeeb0613}

Figure \ref{fig:orgf06deaa} illustrates QQ plot of the model residuals. The observer may say that the 
...
\caption{\label{fig:orgf06deaa}
Normality plot of residuals.}
\end{figure}

\subsubsection{Fitted Against Residuals}
\label{sec:org03ac459}

Figure \ref{fig:org3ce4edd} illustrates the fitted values \(\hat y_j\) against the R-student residuals. No apparent 
...
\caption{\label{fig:org3ce4edd}
Fitted values against R-student residuals.}
\end{figure}

\subsubsection{Added Variable Analysis}
\label{sec:orge61cf79}

Partial regression plots are found in figure \ref{fig:org637ee47}, \ref{fig:orgbc89f03},
\ref{fig:org26ab1d7}, and \ref{fig:org3081a20}. All figures exhibits potential outliers 
(which will be further considered in section \ref{sec:org8e7256d}).
More specifically, in figure \ref{fig:org637ee47} we note a 
few potential outliers on the right hand side of the plot for the \texttt{biceps} regressor, and on the
right and left hand side for the \texttt{forearm} regressor. Moreover, in figure \ref{fig:orgbc89f03}, we 
notice outliers on the right hand side of the \texttt{ankle} plot, and a group of potential outliers on the
\texttt{thigh} plot. Finally, we notice a few potential outliers in figure \ref{fig:org26ab1d7} and 
\ref{fig:org3081a20}.

Figure \ref{fig:orgbc89f03}, \ref{fig:org26ab1d7}, and \ref{fig:org3081a20} 
...
\caption{\label{fig:org3081a20}
Partial regression plots of regressors \texttt{chest}, \texttt{abdomen}, and \texttt{hip}.}
\end{figure}
\subsection{Transformations of variables}
\label{sec:orgc1360e4}

Figure \ref{fig:org6760cc5} displays the values of \(\lambda\) to be used in a potential Box-Cox transformation of 
the dependent variable \texttt{density}. The \(\lambda\) that maximized the log-likelihood is 0.9 (0.7-1.1 95\% CI). 

Using \(\lambda = 0.9\) gives us the normal probability plot displayed on the right hand side in figure \ref{fig:org6760cc5}.
...
In section \ref{sec:orgf23f425} we noted that there was no indication that a transformation was needed. 
...
\caption{\label{fig:org6760cc5}
Values for lambda against the log-likelihood of \texttt{density} for Box-Cox transformations.}
\end{figure}

\subsection{Diagnostics and handling of Outliers}
\label{sec:orga55af72}

Figure \ref{fig:org389137b} illustrates Cook's distance for all points, where the three observations with the largest 
...
\caption{\label{fig:org389137b}
Cook's distance for all observations.}
\end{figure}

Figure \ref{fig:org2c5cfcf} reports the \(DFFITS\) values. We label observations as in figure \ref{fig:org389137b}. We observe 
...
Figure \ref{fig:org6b1e52d}, \ref{fig:orgb4e1971}, \ref{fig:org1ecaa9b}, and
\ref{fig:orgfb40c21} presents \(DFBETA\) values for groups of regressors. Observation 39
...
A coorelation matrix for the full model is found in section \ref{sec:org32aeefc}. The strong multicolinearity
associated with the \texttt{weight} regressor is apparent in the correlation matrix in figure
\ref{fig:org08fe287}. The \texttt{weight} regressor shows a strong correlation with all but the \texttt{age} and
...
\label{sec:org151a884}

The full results of the best subsets regression is presented in section \ref{sec:orgb8e51dc}. The model corresponding
to each model index in Table 3 in \ref{sec:orgb8e51dc} is presented in Table 4. The best subset plots 
are presented in figure \ref{fig:org595ba69}. 


\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{/home/ludvigwgerdin/courses/Regression Analysis/regone/cv_apr.png}
\caption{\label{fig:orga730c00}
